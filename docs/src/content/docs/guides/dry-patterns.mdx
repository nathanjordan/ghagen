...
```typescript
import { checkout, setupNode, setupPython } from "@ghagen/ghagen";
// Note: cache, upload_artifact, download_artifact, setup_uv
// are not yet available as helpers in TypeScript. Use step() directly.
```
</TabItem>
</Tabs>
...
```typescript
checkout({ name: "Checkout with submodules", with_: { submodules: "recursive" } })
setupPython({ version: "3.12", if_: "github.event_name == 'push'" })
```
</TabItem>
</Tabs>
