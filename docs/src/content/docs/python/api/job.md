...
| `version`    | `str \| Raw[str] \| None` | `None`   | Optional version. Enforced at construction against the canonical schema grammar `^\d+(\.\d+\|\*)?$` (`1`, `1.2`, `1*`); patch versions are rejected. Wrap in `Raw(...)` to bypass the grammar. |
