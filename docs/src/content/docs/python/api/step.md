...
| `run`               | `str \| None`                   | `None`  | Shell command(s) to run. Multi-line strings are automatically dedented by default at emission (`to_yaml`/`to_data`), not when the field is set; pass `auto_dedent=False` to keep the string as authored. Mutually exclusive with `uses`. |
