...
Mark your repository root with an empty `.ghagen.yml`. Root discovery walks up the tree looking for
it; without it, generated headers fall back to absolute source paths, which are machine-specific and
break `ghagen check-synced` in CI.
...
Put the definitions in `.github/ghagen_workflows.py`. The CLI looks for a module-level `app` (or a
`create_app()` function) and calls `synth()` for you:
...
Put the definitions in `.github/ghagen.workflows.ts`. The `app` must be **exported** — an
unexported `const app` is invisible to the CLI, which fails with
`must export 'app = new App(...)' or 'createApp(): App'`:
...
`ghagen synth` writes `.github/workflows/ci.yml`:
...
The TypeScript snippet emits the same bytes; only the header's source path differs
