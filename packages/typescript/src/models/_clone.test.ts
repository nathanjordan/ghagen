...
import { cloneModel, isModel, isRaw, isCommented, raw, withEolComment, Model } from "./_base.js";
...
describe("cloneModel()", () => {
  it("returns a Model with the same kind and spec", () => {
    const original = new Model(STEP_SPEC, { run: "echo hi" }, { eolComment: "x" });
    const cloned = cloneModel(original);
    expect(isModel(cloned)).toBe(true);
    expect(cloned.kind).toBe(original.kind);
    expect(cloned.spec).toBe(original.spec);
  });

  it("performs a deep copy of data", () => {
    const original = new Model(STEP_SPEC, { with: { a: "1" } }, {});
    const cloned = cloneModel(original);
  ...
  it("preserves the postProcess function by reference", () => {
    const fn = () => {};
    const original = new Model(WORKFLOW_SPEC, {}, { postProcess: fn });
    const cloned = cloneModel(original);
  ...
  it("preserves Raw values inside data", () => {
    const r = raw("custom");
    const original = new Model(STEP_SPEC, { shell: r }, {});
    const cloned = cloneModel(original);
  ...
  it("recursively clones nested Models inside data arrays", () => {
    const inner = new Model(STEP_SPEC, { run: "echo hi" }, {});
    const outer = new Model(JOB_SPEC, { steps: [inner] }, {});
